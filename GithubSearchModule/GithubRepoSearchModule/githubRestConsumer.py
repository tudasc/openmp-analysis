...
from GithubRepoSearchModule.searchQueryConsumer import SearchQueryConsumer
from GithubRepoSearchModule.githubMetadata import GithubMetadata
import pandas as pd
...
class GithubRestConsumer(SearchQueryConsumer):
    ...
    def _recursiveSearch(self, nextQuery):
        ...
        for i in range(total):
            ...
            githubMetadata = GithubMetadata(name=name, language=language, stars=starCount, cloneUrl=cloneUrl, pushDate=pushDate)
    ...
    def reportRepos(self):
        ...
        cloneUrls = []
        pushDates = []
        for full_name in self._repoNameList:
            full_names.append(full_name)
            metadata = self._githubMetadataDict[full_name]
            names.append(metadata.name)
            languages.append(metadata.language)
            types.append('git')
            starCounts.append(metadata.stars)
            cloneUrls.append(metadata.cloneUrl)
            pushDates.append(metadata.pushDate)
        return pd.DataFrame({"Code":full_names, "Language":languages, "Type":types, "URL": cloneUrls, "Stars": starCounts, "PushDate": pushDates})
